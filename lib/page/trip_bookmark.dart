...
class TripBookMark extends StatefulWidget {
  const TripBookMark({super.key});

  @override
  State<TripBookMark> createState() => _TripBookMarkState();
}

class _TripBookMarkState extends State<TripBookMark> {
  ...
  Stream<QuerySnapshot<Object?>> selectTriptype(String query) {
    if (query == 'ทั้งหมด') {
      return db
          .collection('trip')
          .where('bookmark', arrayContains: my_email)
          .snapshots();
    } else {
      return db
          .collection('trip')
          .where(Filter.and(Filter('bookmark', arrayContains: my_email),
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: Text('รายการทริปที่บันทึกไว้'),
                                                    ...
                                                    onTap: () async {
                                                      QuickAlert.show(
                                                        ...
                                                        title:
                                                            'ยกเลิกบันทึกทริปนี้?',
                                                        text:
                                                            'คุณต้องการยกเลิกบันทึกทริปนี้หรือไม่?',
